...
@Test
public class PrefixLengthFilterTests {
    ...
    public void init() throws Exception {
        ...
        writer.addDocument(tokensToDoc("aa", "aa"));
        writer.addDocument(tokensToDoc("aaa", "aaa"));
        writer.addDocument(tokensToDoc("aaaa", "aaaa"));
        writer.close();
    ...
    public void test() throws Exception {
        Filter filter = new PrefixLengthFilter(new Term("raw", "a"), 2, 3);

        TopDocs results = searcher.search(new FilteredQuery(new MatchAllDocsQuery(), filter), 10);
        assertThat(results.totalHits, equalTo(2));
        Set<String> valuesToBeSeen = new TreeSet<String>(Arrays.asList("aa", "aaa"));
        for (ScoreDoc scoreDoc : results.scoreDocs) {
            Document doc = reader.document(scoreDoc.doc);
            String[] values = doc.getValues("raw");
            assertThat(values, not(equalTo(null)));
