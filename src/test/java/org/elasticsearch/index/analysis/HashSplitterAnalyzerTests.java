...
@Test
public class HashSplitterAnalyzerTests {

    private HashSplitterAnalyzer analyzer;
    ...
    public void testDefaultAnalysis() throws Exception {
        analyzer = new HashSplitterAnalyzer();

        analyze("0123456789");
        for (int i = 0 ; i < input.length() ; ++i) {
            assertThat("at i = " + i, stream.incrementToken(), equalTo(true));
            assertThat("at i = " + i, termAttr.toString(), equalTo(HashSplitterAnalyzer.DEFAULT_PREFIXES.substring(i,i+1) + input.substring(i,i+1)));
    ...
    public void testChunkLength() throws Exception {
        analyzer = new HashSplitterAnalyzer(2);

        analyze("0123456789");
        for (int i = 0 ; i < input.length() ; i += 2) {
            assertThat("at i = " + i, stream.incrementToken(), equalTo(true));
            assertThat("at i = " + i, termAttr.toString(), equalTo(HashSplitterAnalyzer.DEFAULT_PREFIXES.substring(i/2,i/2+1) + input.substring(i,i+2)));
            assertThat("at i = " + i, offAttr.startOffset(), equalTo(i));
    ...
    public void testPrefixes() throws Exception {
        ...
        analyzer = new HashSplitterAnalyzer(HashSplitterAnalyzer.DEFAULT_CHUNK_LENGTH, prefixes);
